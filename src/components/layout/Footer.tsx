...
export default function Footer() {
  return (
    <footer style={{ background: '#111', color: '#aaa', paddingTop: 56 }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 48 }} className="footer-grid">
        ...
        <div>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.7, color: '#888', maxWidth: 280 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
            {['WhatsApp', 'Telegram', 'Instagram', 'Twitter'].map(s => (
              <div key={s} style={{ width: 36, height: 36, background: '#222', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#666', cursor: 'pointer' }}>
                {s[0]}
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          {categories.map(c => (
            <Link key={c.href} href={c.href} style={{ display: 'block', color: '#888', textDecoration: 'none', fontSize: 14, marginBottom: 10, transition: 'color 0.15s' }}
              onMouseOver={e => (e.target as HTMLElement).style.color = '#D32F2F'}
      ...
      <div style={{ maxWidth: 1200, margin: '48px auto 0', padding: '20px 24px', borderTop: '1px solid #222', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        <p style={{ fontSize: 13, color: '#555' }}>© {new Date().getFullYear()} Future Finder. All rights reserved.</p>
        <p style={{ fontSize: 13, color: '#555' }}>Built for African Youth 🌍</p>
      </div>
